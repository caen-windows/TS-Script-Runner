<#
    .DESCRIPTION
        ...
        This is a script from www.configjon.com which has been modified for CAEN use.
        WMI Status Codes
            0 - Success
            1 - Failed
            2 - Invalid Parameter
...
## Part of MOD1 next 4 lines
    .PARAMETER PFile
        Specify the file containing the current BIOS password
    .PARAMETER PKey
        Key for password file
    .EXAMPLE
    ...
    .CHANGELOG
        ...
        2021-06-28 - CAEN MOD#1 to support encrypted passwords
...
## Part of MOD#1 next 2 lines
    [Parameter(Mandatory=$false)][ValidateNotNullOrEmpty()][String]$PFile,
    [Parameter(Mandatory=$false)][ValidateNotNullOrEmpty()][String]$PKey
)
...
# Derive admin password from file and key if they exist
# Convert key to bytes
if($PFile -and $PKey) {
    # Read in pfile
    $ct = Get-Content $PFile

    # Convert key hex string to byte array
    $key = [byte[]]::new($PKey.length/2)
    for($i=0; $i -lt $PKey.length; $i+=2){$key[$i/2] = [convert]::ToByte($PKey.substring($i, 2), 16)}

    # Decrypt file
    # Note: Key must be 128, 192 or 256 bytes in length
    $ss = ConvertTo-SecureString -String $ct -Key $key
